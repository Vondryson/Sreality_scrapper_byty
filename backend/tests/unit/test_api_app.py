...
async def request_many(app: FastAPI, *paths: str) -> list[httpx.Response]:
    transport = httpx.ASGITransport(app=app, raise_app_exceptions=False)
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        return [await client.get(path) for path in paths]


async def patch_json(app: FastAPI, path: str, payload: dict[str, object]) -> httpx.Response:
    transport = httpx.ASGITransport(app=app, raise_app_exceptions=False)
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
...
def test_listing_range_validation_uses_uniform_error_contract() -> None:
    ...
    [response] = asyncio.run(request_many(app, "/api/v1/listings?price_min=200&price_max=100"))
